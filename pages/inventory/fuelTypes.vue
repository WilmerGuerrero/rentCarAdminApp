...
const onSubmit = async (formData: BaseInventoryFields) => {
  ...
  const save = await useFetch<FuelTypes>("/api/fuelTypes/put", {
    method: "PUT",
  ...
  //   save.execute()
  if (save.error) clearError({ redirect: "/inventory" });
...
<template>
  ...
  <dynamic-table :columns="tableColumns" :data="get.data.value || []">
  </dynamic-table>
</template>
